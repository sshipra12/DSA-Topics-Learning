    ...
    vector<int> productExceptSelf(vector<int>& nums) {
        ...
        for(i=0;i<=nums.size()-1;i++){
            if(i==0)
                ...
                ans[i]=prefix[i-1];
            else ans[i]=prefix[i-1]*suffix[i+1];  // NOTE: not taking into the case where given nums array is of 1 size
                                                  // then we should also check for cnd if(i==0&&i==nums.size()-1), i.e index is beginning and ending at the same time
        }
